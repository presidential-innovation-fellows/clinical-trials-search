...
async function renderPage(page, component) {
  ...
  let file = join(__dirname, '../build', page.file.substr(0, page.file.lastIndexOf('.')) + '.html');
  let html = '<!doctype html>\n' + ReactDOM.renderToStaticMarkup(<Html debug={DEBUG} {...data} />);
  await fs.mkdir(dirname(file));
  await fs.writeFile(file, html);
  if (!page.path.endsWith('/')) {
    file = join(__dirname, '../build', page.file.substr(0, page.file.lastIndexOf('.')) + '/index.html');
    await fs.mkdir(dirname(file));
    await fs.writeFile(file, html);
  }
}
